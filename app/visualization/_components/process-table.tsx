...
const ProcessTable = ({ data, averageWaitingTime, averageTurnaroundTime }: IProps) => {
  return (
    <div className="grid border border-emerald-500/30 rounded-lg">
        ...
        <TableBody>
          {data.map((p: Process) => (
            <TableRow key={p.id} className="hover:bg-emerald-950/30 border-emerald-500/30">
              <TableCell className="text-center text-white font-medium">{p.name}</TableCell>
          ...
          <TableRow className="hover:bg-emerald-950/30 border-emerald-500/30">
            <TableCell className="font-bold text-center text-emerald-400" colSpan={2}>
              Rata-rata
            </TableCell>
            <TableCell className="font-bold text-center text-emerald-400">{averageWaitingTime.toFixed(2)}</TableCell>
            <TableCell className="font-bold text-center text-emerald-400">{averageTurnaroundTime.toFixed(2)}</TableCell>
          </TableRow>
        </TableBody>
      </Table>
    </div>
